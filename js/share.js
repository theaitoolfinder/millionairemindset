...
(function(){
  ...
  /* Same pill-button system used across myaitoolsfinder.com articles:
     outlined, monochrome, brand-tinted fill on hover. */
  var CSS = `
  .mm-share-bar {
    display: flex;
    align-items: center;
  ...
  .mm-share-btn {
    ...
    font-size: 0.78rem;
    font-weight: 600;
    color: var(--text-2, var(--text-dim, #555));
    background: none;
  ...
  .mm-share-btn:hover {
    border-color: var(--primary, #CC1010);
    color: var(--primary, #CC1010);
    background: var(--primary-light, #fdecea);
  }
  .mm-share-btn svg { flex-shrink: 0; }
  .mm-share-btn.copied { border-color: #a7f3d0; color: #065f46; background: #d1fae5; }
  `;
  ...
  window.mmShareBar = function(containerId, opts) {
    ...
    if (navigator.share) {
      ...
      nativeBtn = '<button class="mm-share-btn" '
    ...
    bar.innerHTML =
      ...
      + nativeBtn
      + '<a class="mm-share-btn" href="https://www.facebook.com/sharer.php?u=' + encodedUrl(url) + '" target="_blank" rel="noopener">'
        + '<svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>Facebook</a>'
      + '<a class="mm-share-btn" href="https://twitter.com/intent/tweet?url=' + encodedUrl(url) + '&text=' + encodedText(title) + '" target="_blank" rel="noopener">'
        + '<svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>X</a>'
      + '<a class="mm-share-btn" href="https://wa.me/?text=' + encodedText(msg) + '" target="_blank" rel="noopener">'
      ...
      + '<a class="mm-share-btn" href="viber://forward?text=' + encodedText(msg) + '" target="_blank" rel="noopener">'
        + '<svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M11.4 2C6.12 2 2 6.12 2 11.4c0 2.6 1.06 4.96 2.77 6.68L3.5 22l4.13-1.08A9.3 9.3 0 0011.4 21.8c5.28 0 9.4-4.12 9.4-9.4S16.68 2 11.4 2zm4.36 13.06c-.23.65-1.34 1.24-1.84 1.3-.46.06-.9.08-2.93-.61-2.46-.84-4.04-3.36-4.16-3.52-.12-.16-.98-1.3-.98-2.48s.62-1.76.84-2c.22-.24.48-.3.64-.3l.46.01c.14 0 .34-.05.53.4l.76 1.86c.07.17.04.37-.07.53l-.42.52-.16.17c.09.17.44.72 1.02 1.24.72.65 1.33.86 1.52.95.19.1.3.08.41-.05l.56-.67c.12-.15.3-.19.48-.12l1.83.86c.2.1.32.15.34.25.03.1.03.54-.23 1.16z"/></svg>Viber</a>'
      + '<a class="mm-share-btn" href="https://t.me/share/url?url=' + encodedUrl(url) + '&text=' + encodedText(title) + '" target="_blank" rel="noopener">'
        + '<svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0a12 12 0 00-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 01.171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>Telegram</a>'
      + '<button class="mm-share-btn" id="mmCopyBtn_' + Math.random().toString(36).slice(2) + '" onclick="mmCopyLink(this,\'' + url.replace(/'/g,"\\'") + '\')">'
        + '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>Copy Link</button>';
